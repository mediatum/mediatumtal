...
def unicode_pizza():
    ...
    pizza.reset_mock()
    pizza.get_size.return_value = 13
    pizza.get_toppings.return_value = [u"tömatoeß", u"mushrooms"]
    return pizza
...
expected_unicode_pizza_write_calls = copy(expected_pizza_write_calls)
expected_unicode_pizza_write_calls[9] = call(u'tömatoeß')
...
def test_tal(writer, pizza):
    runTAL(writer, {"pizza": pizza}, file=_path("pizza.html"))
    assert writer.write.call_args_list == expected_pizza_write_calls


def test_unicode_tal(writer, unicode_pizza):
    runTAL(writer, {"pizza": unicode_pizza}, file=_path("pizza.html"))
    assert writer.write.call_args_list == expected_unicode_pizza_write_calls


if __name__ == "__main__":
    test_tal_example_macro(writer(), pizza())
    test_tal(writer(), pizza())
    test_unicode_tal(writer(), unicode_pizza())
